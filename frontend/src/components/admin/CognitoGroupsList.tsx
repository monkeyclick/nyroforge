...
const CognitoGroupsList: React.FC = () => {
  ...
  const handleCreateGroup = async (groupData: { groupName: string; description?: string; precedence?: number }) => {
    setCreating(true);
    try {
      await apiClient.createCognitoGroup(groupData);
      await loadGroups();
  ...
  const handleDeleteGroup = async (groupName: string) => {
    if (!confirm(`Are you sure you want to delete the group "${groupName}"? Users in this group will lose the access it grants. This action cannot be undone.`)) {
      return;
    }

    setDeleting(groupName);
    try {
      await apiClient.deleteCognitoGroup(groupName);
      await loadGroups();
    } catch (err: any) {
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Create a group to organize users and control workstation access
        </p>
        <button
        ...
        <div className="flex">
          ...
          <div className="ml-3 flex-1">
            ...
            <div className="mt-2 text-sm text-blue-700">
              <p>
                These groups control access and package assignments. Add or remove members from a
                user&apos;s edit dialog in the Users tab.
              </p>
            </div>
          </div>
        </div>
